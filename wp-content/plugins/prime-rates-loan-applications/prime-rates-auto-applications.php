<?php
define( 'PRIME_RATES_AUTO_APPLICATIONS_PLUGIN_URL', PRIME_RATES_LOAN_APPLICATIONS_PLUGIN_URL.'auto-app'.DIRECTORY_SEPARATOR);
define( 'PRIME_RATES_AUTO_APPLICATIONS_JS_URL', PRIME_RATES_AUTO_APPLICATIONS_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR );

function auto_apply_for_loans_shortcode($atts = []){
...
function prime_rates_auto_applications_enqueue() {
	global $post, $versionConfigData;
	if(has_shortcode( $post->post_content, 'auto-application')){
	  wp_enqueue_script('prime_rates_ppp_applications', PRIME_RATES_AUTO_APPLICATIONS_JS_URL.'main.2fe7c868.js', array(), 0,false );
	  ...
	  //$_SESSION['isAuto'] = 1;	
	  setcookie('isAuto','1',0,'/','api-dataview360.com');
				...
				if(isset($host['hosts']) && is_array($host['hosts'])){
					foreach($host['hosts'] as $host_url){
						if($_SERVER['HTTP_HOST'] == $host_url['host']){
							wp_enqueue_style($host['slug'].'_auto_css',get_stylesheet_directory_uri().'/css/Breadcrumb/'.$host['auto_css']);
							wp_enqueue_style($host['slug'].'_auto_transition_css',get_stylesheet_directory_uri().'/css/'.$host['auto_transition_css']);
						}
					}
				}
			}
...
function prime_rates_auto_applications_shortcodes_init(){
	add_shortcode('auto-application', 'auto_apply_for_loans_shortcode');
}

add_action('init', 'prime_rates_auto_applications_shortcodes_init');
add_action( 'wp_enqueue_scripts', 'prime_rates_auto_applications_enqueue', 99);
