<?php
define( 'GDS_CREDITCARD_PLUGIN_URL', PRIME_RATES_LOAN_APPLICATIONS_PLUGIN_URL.'gds-cc'.DIRECTORY_SEPARATOR);
define( 'GDS_CREDITCARD_JS_URL', GDS_CREDITCARD_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR );

function gds_cc_apply_for_loans_shortcode($atts = []){
...
function gds_creditcard_enqueue() {
	global $post, $versionConfigData;
	if(has_shortcode( $post->post_content, 'gds-creditcard')){
	  wp_enqueue_script('gds_cc_applications', GDS_CREDITCARD_JS_URL.'main.d9d11c61.js', array(), 0,false );
...
function gds_creditcard_shortcodes_init(){
	add_shortcode('gds-creditcard', 'gds_cc_apply_for_loans_shortcode');
}

add_action('init', 'gds_creditcard_shortcodes_init');
add_action( 'wp_enqueue_scripts', 'gds_creditcard_enqueue', 99);
