<?php
define( 'PRIME_RATES_CC_OFFERS_PAGE_PLUGIN_URL', PRIME_RATES_LOAN_APPLICATIONS_PLUGIN_URL.'uk-cc-offers'.DIRECTORY_SEPARATOR);
define( 'PRIME_RATES_CC_OFFERS_PAGE_CSS_URL', PRIME_RATES_CC_OFFERS_PAGE_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'css'.DIRECTORY_SEPARATOR );
define( 'PRIME_RATES_CC_OFFERS_PAGE_JS_URL', PRIME_RATES_CC_OFFERS_PAGE_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR );

function pre_qualified_creditcard_offers_shortcode(){
	...
	if($_POST){
		if(isset($_SESSION['multi_step_data'])){
			...
			makeRequestToDV360CC('pre-qualify',$allData);
			unset($_SESSION['multi_step_data']);
		}else{
			makeRequestToDV360CC('pre-qualify',$_POST);
		}

	}else{
		if(isset($_SESSION[session_id().'-formData']) && !empty($_SESSION[session_id().'-formData'])){
			makeRequestToDV360CC('pre-qualify',$_SESSION[session_id().'-formData']);
		}else{
...
function makeRequestToDV360CC($parentPage,$postData){
	...
	?>
		...
		<link href="<?php echo PRIME_RATES_CC_OFFERS_PAGE_CSS_URL; ?>main.097b838a.css" rel="stylesheet">
		<link href="<?php echo get_stylesheet_directory_uri() ?>/css/range-slider.css" rel="stylesheet">
		<?php if(strpos($_SERVER['REQUEST_URI'],'gds-link/credit-cards') !== false){ ?>
...
function uk_creditcard_offers_page_enqueue() {
	global $post;
	if(has_shortcode( $post->post_content, 'uk-creditcard-offers') && ($_POST || (isset($_SESSION['isOfferAvailable']) && $_SESSION['isOfferAvailable'] == 1))){
		$action = 'mandrill_incomplete';
	  wp_enqueue_script('prime_rates_loan_applications', PRIME_RATES_CC_OFFERS_PAGE_JS_URL.'main.d9bb1e56.js', array(), 0,false );  
	} 
}

function uk_creditcard_offers_page_shortcodes_init(){
	add_shortcode('uk-creditcard-offers', 'pre_qualified_creditcard_offers_shortcode');
}


add_action('init', 'uk_creditcard_offers_page_shortcodes_init');
add_action( 'wp_enqueue_scripts', 'uk_creditcard_offers_page_enqueue' );
