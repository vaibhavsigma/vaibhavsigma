<?php
define( 'PRIME_RATES_PPP_APPLICATIONS_PLUGIN_URL', PRIME_RATES_LOAN_APPLICATIONS_PLUGIN_URL.'ppp-app'.DIRECTORY_SEPARATOR);
define( 'PRIME_RATES_PPP_APPLICATIONS_JS_URL', PRIME_RATES_PPP_APPLICATIONS_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR );

function ppp_apply_for_loans_shortcode($atts = []){
...
function prime_rates_ppp_applications_enqueue() {
	global $post, $versionConfigData;
	if(has_shortcode( $post->post_content, 'ppp-application')){
	  wp_enqueue_script('prime_rates_ppp_applications', PRIME_RATES_PPP_APPLICATIONS_JS_URL.'main.6b51ff84.js', array(), 0,false );
...
function prime_rates_ppp_applications_shortcodes_init(){
	add_shortcode('ppp-application', 'ppp_apply_for_loans_shortcode');
}

add_action('init', 'prime_rates_ppp_applications_shortcodes_init');
add_action( 'wp_enqueue_scripts', 'prime_rates_ppp_applications_enqueue', 99);
