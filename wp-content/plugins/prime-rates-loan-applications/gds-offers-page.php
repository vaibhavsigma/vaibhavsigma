<?php
define( 'PRIME_RATES_GDS_OFFERS_PAGE_PLUGIN_URL', PRIME_RATES_LOAN_APPLICATIONS_PLUGIN_URL.'gds-offers'.DIRECTORY_SEPARATOR);
define( 'PRIME_RATES_GDS_OFFERS_PAGE_CSS_URL', PRIME_RATES_GDS_OFFERS_PAGE_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'css'.DIRECTORY_SEPARATOR );
define( 'PRIME_RATES_GDS_OFFERS_PAGE_JS_URL', PRIME_RATES_GDS_OFFERS_PAGE_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR );

function pre_qualified_gds_offers_shortcode(){
	...
	if($_POST){
		if(isset($_SESSION['multi_step_data'])){
			...
			makeRequestToDV360GDS('pre-qualify',$allData);
			unset($_SESSION['multi_step_data']);
		}else{
			makeRequestToDV360GDS('pre-qualify',$_POST);
		}

	}else{
		if(isset($_SESSION[session_id().'-formData']) && !empty($_SESSION[session_id().'-formData'])){
			makeRequestToDV360GDS('pre-qualify',$_SESSION[session_id().'-formData']);
		}else{
...
function makeRequestToDV360GDS($parentPage,$postData){
    ...
    if (!$isOfferAvailable || isset($_GET['applicationid'])){ // Added additional condition for Headway dealers to lookup another application id withing same user session - PM-730 - 08-08-2018
		...
		// echo "</pre>";
			...
			$curl = new Curl();
			if(strpos($_SERVER["HTTP_HOST"],"los-qa.api-dataview360.com") !== false || strpos($_SERVER["HTTP_HOST"],"qa-coopertiva.api-dataview360.com") !== false){
			//$url = "https://projectdrive-dv360-uat.api-dataview360.com/transaction";
			...
			if($data['TransactionType'] == "reusable_offers_check"){
				...
				if(strpos($_SERVER["HTTP_HOST"],"los-qa.api-dataview360.com") !== false || strpos($_SERVER["HTTP_HOST"],"qa.api-dataview360.com") !== false){
				...
				$_SESSION['application_response'] = json_decode(json_encode($result->response), true);
	...
	?>
		...
		<link href="<?php echo PRIME_RATES_GDS_OFFERS_PAGE_CSS_URL; ?>main.097b838a.css" rel="stylesheet">
...
function gds_offers_page_enqueue() {
	global $post;
	if(has_shortcode( $post->post_content, 'gds-offers') && ($_POST || (isset($_SESSION['isOfferAvailable']) && $_SESSION['isOfferAvailable'] == 1))){
		//$action = 'mandrill_incomplete';
	  wp_enqueue_script('prime_rates_loan_applications', PRIME_RATES_GDS_OFFERS_PAGE_JS_URL.'main.e5fb6916.js', array(), 0,false );  
	} 
}

function gds_offers_page_shortcodes_init(){
	add_shortcode('gds-offers', 'pre_qualified_gds_offers_shortcode');
}


add_action('init', 'gds_offers_page_shortcodes_init');
add_action( 'wp_enqueue_scripts', 'gds_offers_page_enqueue' );
