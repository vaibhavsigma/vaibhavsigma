<?php
define( 'PRIME_RATES_UK_CREDITCARD_PLUGIN_URL', PRIME_RATES_LOAN_APPLICATIONS_PLUGIN_URL.'uk-cc'.DIRECTORY_SEPARATOR);
define( 'PRIME_RATES_UK_CREDITCARD_JS_URL', PRIME_RATES_UK_CREDITCARD_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR );

function uk_cc_apply_for_loans_shortcode($atts = []){
...
function prime_rates_uk_creditcard_enqueue() {
	global $post, $versionConfigData;
	if(has_shortcode( $post->post_content, 'uk-creditcard')){
	  wp_enqueue_script('prime_rates_uk_applications', PRIME_RATES_UK_CREDITCARD_JS_URL.'main.658974e6.js', array(), 0,false );
	  ...
	  if(strpos($_SERVER['REQUEST_URI'],'gds-link/credit-cards') !== false){
		  wp_enqueue_style($host['slug'].'_breadcrumb_css',get_stylesheet_directory_uri().'/css/Breadcrumb/BCStyles-los-uk-creditcard.css');
		  wp_enqueue_style($host['slug'].'_breadcrumb_transition_css',get_stylesheet_directory_uri().'/css/transition-gdsbank.css');
	  }else{
...
function prime_rates_uk_creditcard_shortcodes_init(){
	add_shortcode('uk-creditcard', 'uk_cc_apply_for_loans_shortcode');
}

add_action('init', 'prime_rates_uk_creditcard_shortcodes_init');
add_action( 'wp_enqueue_scripts', 'prime_rates_uk_creditcard_enqueue', 99);
