<?php
define( 'PRIME_RATES_UK_APPLICATIONS_PLUGIN_URL', PRIME_RATES_LOAN_APPLICATIONS_PLUGIN_URL.'uk-app'.DIRECTORY_SEPARATOR);
define( 'PRIME_RATES_UK_APPLICATIONS_JS_URL', PRIME_RATES_UK_APPLICATIONS_PLUGIN_URL.'static'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR );

function uk_apply_for_loans_shortcode($atts = []){
...
function prime_rates_uk_applications_enqueue() {
	global $post, $versionConfigData;
	if(has_shortcode( $post->post_content, 'uk-application')){
	  wp_enqueue_script('prime_rates_uk_applications', PRIME_RATES_UK_APPLICATIONS_JS_URL.'main.25dd1088.js', array(), 0,false );
...
function prime_rates_uk_applications_shortcodes_init(){
	add_shortcode('uk-application', 'uk_apply_for_loans_shortcode');
}

add_action('init', 'prime_rates_uk_applications_shortcodes_init');
add_action( 'wp_enqueue_scripts', 'prime_rates_uk_applications_enqueue', 99);
